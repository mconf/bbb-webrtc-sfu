...
class LiveKitBbbManager {
   ...
   * @param {object} event - The normalized track_unpublished event.
   * @param {object} participantMetadata - The parsed metadata of the participant.
   * @param {object} [room] - The cached room the track was unpublished from.
   * @private
   */
  _handleScreenShareTrackUnpublished(event, participantMetadata, room) {
    // Room purpose check note: see _handleCameraTrackPublished.
    try {
      const voiceConf = participantMetadata?.voiceConf
        || room?.bbbParsedMetadata?.bbb_voiceConf
        || room?.bbbParsedMetadata?.voiceConf;

      if (!voiceConf) {
        Logger[room ? 'warn' : 'debug']('LiveKitBbbManager: screen share stop has no voiceConf, not sent', {
          roomName: event.room?.name,
          trackSid: event.track?.sid,
        });
        return;
    ...
    try {
      ...
      switch (trackSource) {
        ...
        case 'screen_share_audio':
          this._handleScreenShareTrackUnpublished(event, participantMetadata, room);
          break;
        default:
