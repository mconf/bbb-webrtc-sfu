...
module.exports = class Media extends EventEmitter {
  constructor(
    ...
    this.enableSubtitle = options.enableSubtitle;
  ...
  /**
   * Tells whether media's subtitle is enabled or not
   * @type {Boolean}
   */
  get enableSubtitle () {
    return this._enableSubtitle;
  }

  set enableSubtitle (value) {
  ...
  async trackMedia () {
    ...
    const ADAPTER_EVENTS_TO_LISTEN = [
      ...
      { eventType: C.EVENT.MEDIA_DTMF, callback: this._handleDtmfEvent.bind(this) },
  ...
  /**
   * Handle dtmf triggered by the adapter
   * Unlikely other media events, this event is not only dispatched to Global
   * Event Emitter, but is also emited to media listeners
   * (such as media-session)
   * @return {Promise} A Promise for the given process
   */
  async _handleDtmfEvent (event) {
    try {
      let dtmfDigit = event ? event.dtmfDigit : -1;
      this._dispatchDtmfEvent(dtmfDigit);
      this.emit(C.EVENT.MEDIA_DTMF, dtmfDigit);
    } catch (error) {
   ...
   * @return {Promise} A promise for the given process.
   */
  async createHubPort (enableSubtitle, subtitle) {
    try {
      return this.adapter.createHubPort(this.adapterElementId, enableSubtitle,
        subtitle);
    } catch (error) {
      throw (this._handleError(error));
    }
  ...
  _dispatchDtmfEvent (dtmfDigit) {
    try {
      ...
      Logger.info(LOG_PREFIX,"Dispatching media dtmf", event);
      GLOBAL_EVENT_EMITTER.emit(C.EVENT.MEDIA_DTMF, event);
    } catch (error) {
