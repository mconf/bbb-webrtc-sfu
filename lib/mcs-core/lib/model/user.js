...
module.exports = class User {
  constructor(roomId, type, params = {}) {
    ...
    this.subtitle = params.subtitle ? params.subtitle : this.id;
    this._mediaSessions = {};
  ...
  getContentMedias() {
    let userContentMedias = [];
    Object.keys(this._mediaSessions).forEach(key => {
      let mediaSession = this._mediaSessions[key];
      for(let media of mediaSession.medias) {
        if(media.mediaTypes.content) {
          userContentMedias.push(media);
        }
      }
    });
    return userContentMedias;
  }
