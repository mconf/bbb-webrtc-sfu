...
class EslWrapper extends EventEmitter {
  ...
  _onSubscribed () {
    ...
    this._client.on('esl::event::'+ESL_EVENTS.DTMF+'::*', this._onDtmfEvent.bind(this));
  ...
  /**
   * Handle DTMF Event
   * @param  {Object} event ESL Event Object descriptor
   */
  _onDtmfEvent(event) {
    try {
      let channelId = event.getHeader(ESL_EVENT.CHANNEL_CALL_UUID);
      let dtmfDigit = event.getHeader(ESL_EVENT.DTMF_DIGIT);
      if (channelId && dtmfDigit) {
        this.emit(EVENTS.DTMF, channelId, dtmfDigit);
      }
    } catch (error) {
