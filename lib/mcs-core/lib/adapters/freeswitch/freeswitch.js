...
module.exports = class Freeswitch extends EventEmitter {
  ...
  _trackESLEvents() {
      ...
      this._eslWrapper.on(EslWrapper.EVENTS.DTMF, this._handleDtmf.bind(this));
  }
  ...
  _handleDtmf (channelId, dtmfDigit) {
    const elementId = this._channelIdInfos[channelId].ua;
    Logger.info(LOG_PREFIX,'Emitting User agent',elementId,
      'dtmf event with digit:', dtmfDigit);
    this.emit(C.EVENT.MEDIA_DTMF+elementId, {dtmfDigit: dtmfDigit});
  }
